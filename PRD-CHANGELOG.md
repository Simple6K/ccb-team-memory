...
## 变更 V4.11.1 — 交互选择器 + 时间筛选 + 进度条 + 断点续传

**日期**: 2026-05-19
**版本**: V4.11 → V4.11.1
**变更类型**: 功能增强 — 批量提取交互化 + 增量处理

### 背景

`extract batch` 缺少交互式选择能力（351 个会话无法按需选取）和时间筛选，全量处理无进度反馈且中断后无法续传。

### 变更

**交互式终端选择器**：新增 `picker.py`，基于 stdlib termios/tty 实现箭头键导航、空格选中/取消、全选/全不选、视口滚动。默认进入交互模式，`--no-pick` 跳过。非 TTY 环境（ccb skill 调用）返回提示信息。

**时间筛选**：`--since`/`--until` 参数支持 ISO 日期（`2026-04-01`）和相对时间（`7d`）。按会话首末消息时间戳过滤。

**进度条**：提取过程显示 `[15/351] ██████░░░░ 4.3%` 格式进度。

**断点续传**：`_staging/.batch-state.json` 记录已处理 session 文件名。中途退出后再次运行自动跳过已处理会话。`--reset` 清除状态重新提取。全量提取无重复。

### 修改文件

| 文件 | 改动 |
|------|------|
| `utils/picker.py` | **新建** — TerminalPicker 交互选择器 |
| `utils/transcript.py` | 新增 `get_session_time_range()` + `get_session_summary()` |
| `cli/extract.py` | 添加 `--since`/`--until`/`--no-pick`/`--reset` + 交互选择 + 进度条 + 断点续传 |
| `templates/SKILL.md` | 注册 `extract batch` 命令 |
