...
def install_hooks(project_root: str | Path | None = None, bin_path: str | None = None, global_hooks: bool = True) -> tuple[bool, str]:
    ...
    for event, entries in hooks_to_add.items():
        ...
        for entry in entries:
            # Simple dedup: check if any existing entry has the same command
            already_exists = False
            for existing in existing_hooks[event]:
                existing_commands = [
                ...
                if any(nc in ec or ec in nc for nc in new_commands for ec in existing_commands):
                    already_exists = True
                    break
